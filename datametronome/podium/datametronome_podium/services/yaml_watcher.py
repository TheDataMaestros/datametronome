...
from datametronome_podium.services.yaml_loader import YAMLLoadError, load_and_parse_yaml
...
def reload_yaml_file(file_path: str) -> ReloadResult:
    ...
    try:
        ...
        return ReloadResult(
            success=False, file_path=file_path, error=f"YAML load error: {str(e)}"
        )
    except InterpolationError as e:
        return ReloadResult(
            success=False, file_path=file_path, error=f"Interpolation error: {str(e)}"
        )
    except Exception as e:
        logger.exception(f"Unexpected error reloading {file_path}")
        return ReloadResult(
            success=False, file_path=file_path, error=f"Unexpected error: {str(e)}"
        )
...
def watch_yaml_directory(
    path: str, callback: Optional[Callable[[str], ReloadResult]] = None
):
    """
