...
async def calculate_job_health_metrics(job_id: str, lookback_days: int = 30) -> Optional[JobHealthMetrics]:
    """
    ...
    try:
        ...
        successful_times = [e.execution_time for e in executions if e.status == "success" and e.execution_time]
        ...
        last_success = next((e.started_at for e in executions if e.status == "success"), None)
        last_failure = next((e.started_at for e in executions if e.status == "failure"), None)
...
async def get_failing_jobs(consecutive_failure_threshold: int = 3) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        for job_data in jobs:
            ...
            if metrics and metrics.consecutive_failures >= consecutive_failure_threshold:
                failing_jobs.append({
                    "job_id": job_id,
                    ...
                    "last_failure": metrics.last_failure.isoformat() + "Z" if metrics.last_failure else None
                })
