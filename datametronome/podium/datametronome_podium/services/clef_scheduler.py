    ...
    try:
        ...
        if not clef_result:
            ...
            await record_job_execution(
                job_id, clef_id, "failure", error_message="Clef not found"
            )
        ...
        if not stave_result:
            ...
            await record_job_execution(
                job_id, clef_id, "failure", error_message="Stave not found"
            )
            await increment_job_execution_count(job_id, success=False)
        ...
        if not is_success and retry_attempt == 0:
            ...
            if max_retries > 0:
                ...
                logger.info(
                    f"⏳ Scheduling retry for clef {clef_id} in {delay_seconds} seconds"
                )

                # Schedule retry
                await asyncio.sleep(delay_seconds)
                return await execute_scheduled_clef(
                    clef_id, retry_attempt=retry_attempt + 1
                )

        logger.info(
            f"✅ Scheduled execution completed for clef {clef_id}: {result.severity}"
        )
        return {
            "success": is_success,
            "clef_id": clef_id,
            ...
            try:
                ...
                if clef_result:
                    ...
                    if max_retries > 0:
                        ...
                        logger.info(
                            f"⏳ Scheduling retry for clef {clef_id} after exception in {delay_seconds} seconds"
                        )
                        await asyncio.sleep(delay_seconds)
                        return await execute_scheduled_clef(
                            clef_id, retry_attempt=retry_attempt + 1
...
async def reschedule_clef(clef_id: str) -> bool:
    ...
    try:
        ...
        # Schedule again
        job = add_scheduled_job(
            clef_id=clef.id, schedule=cron_expression, func=execute_scheduled_clef
        )
