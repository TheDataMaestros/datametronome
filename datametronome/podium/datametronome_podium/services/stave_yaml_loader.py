...
from datametronome_podium.services.config_validator import validate_configuration, ConfigurationIssue
...
async def import_staves_from_yaml(
    ...
    for stave in staves:
        ...
        existing = await db.query({
            "sql": "SELECT id FROM staves WHERE id = ?",
            "params": [stave.id]
        })
    ...
    for clef in clefs:
        # Check if exists
        existing = await db.query({
            "sql": "SELECT id FROM clefs WHERE id = ?",
            "params": [clef.id]
        })
    ...
    return {
        "staves": staves_imported,
        "clefs": clefs_imported
    }
