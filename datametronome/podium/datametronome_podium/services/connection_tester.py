...
class ConnectionTester:
    ...
    async def get_connector(self, stave: Stave, read_only: bool = True):
        ...
        try:
            if data_source_type in ["postgres", "postgresql"]:
                ...
                if not db_path:
                    raise RuntimeError("SQLite connection requires 'database_path' or 'path' in connection_config")
                ...
                if read_only:
                    from metronome_pulse_bigquery import BigQueryReadonlyPulse as PulseClass
                else:
                ...
                raise RuntimeError(f"Unsupported data source type: {stave.data_source_type}")
        
        except KeyError as exc:
            raise RuntimeError(f"Missing required connection field: {exc}") from exc
        except ImportError as exc:
            raise RuntimeError(f"Required connector package is not installed: {exc.name}") from exc
    ...
    async def _test_postgres_connection(self, stave: Stave) -> Dict[str, Any]:
        ...
        try:
            ...
            schema_result = await connector.query("SELECT COUNT(*) as count FROM information_schema.schemata;")
            schema_count = schema_result[0]['count'] if schema_result else 0
            
            table_result = await connector.query("SELECT COUNT(*) as count FROM information_schema.tables;")
    ...
    async def _test_sqlite_connection(self, stave: Stave) -> Dict[str, Any]:
        ...
        try:
            ...
            version_result = await connector.query("SELECT sqlite_version() as version;")
            version = version_result[0]['version'] if version_result else "Unknown"
            
            table_result = await connector.query("SELECT COUNT(*) as count FROM sqlite_master WHERE type='table';")
            table_count = table_result[0]['count'] if table_result else 0
            
            index_result = await connector.query("SELECT COUNT(*) as count FROM sqlite_master WHERE type='index';")
        ...
        try:
            ...
            return {
                ...
                "metadata": {
                    ...
                    "uri": config['uri'].replace(config.get('password', ''), '***') if config.get('password') else config['uri']
                }
            }
