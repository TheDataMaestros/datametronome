...
def interpolate_values(data: Any, env: Dict[str, str] = None, strict: bool = True) -> Any:
    """
...
def _interpolate_recursive(data: Any, env: Dict[str, str], strict: bool) -> Any:
    ...
    if isinstance(data, dict):
        return {key: _interpolate_recursive(value, env, strict) for key, value in data.items()}
...
def _interpolate_string(value: str, env: Dict[str, str], strict: bool) -> str:
    ...
    def replace_match(match):
        ...
        if var_name in env:
            ...
            raise InterpolationError(f"Required environment variable '{var_name}' is not set")
        else:
...
def validate_required_vars(yaml_data: Dict[str, Any], env: Dict[str, str] = None) -> List[str]:
    """
...
def interpolate_yaml_data(yaml_data: Dict[str, Any], env: Dict[str, str] = None, strict: bool = True) -> Dict[str, Any]:
    """
