...
def validate_yaml_structure(yaml_data: Dict[str, Any]) -> ValidationResult:
    ...
    if not has_flat_format and not has_nested_format:
        errors.append("YAML must contain either 'staves'/'clefs' (flat format) or 'clef' (nested format)")
        return ValidationResult(False, errors)
            ...
            for i, stave in enumerate(staves):
                if not isinstance(stave, dict):
                    ...
                    if "name" not in stave:
                        errors.append(f"Stave at index {i} missing required field 'name'")
                    if "data_source_type" not in stave:
                        errors.append(f"Stave at index {i} missing required field 'data_source_type'")
        ...
        if not isinstance(clefs, list):
            ...
            for i, clef in enumerate(clefs):
                if not isinstance(clef, dict):
                    ...
                    if "name" not in clef:
                        errors.append(f"Clef at index {i} missing required field 'name'")
                    if "check_type" not in clef:
                        errors.append(f"Clef at index {i} missing required field 'check_type'")
                    if "stave_id" not in clef:
                        errors.append(f"Clef at index {i} missing required field 'stave_id'")
                    ...
                    for i, check in enumerate(checks):
                        if not isinstance(check, dict):
                            ...
                            if "check" not in check:
                                errors.append(f"Check at index {i} missing required field 'check'")
...
def parse_clefs(yaml_data: Dict[str, Any], stave_id_map: Optional[Dict[str, str]] = None) -> List[Clef]:
    """
    ...
    if "clef" in yaml_data:
        ...
        if not default_stave_id and stave_id_map:
            ...
            if len(stave_ids) == 1:
                ...
                raise YAMLLoadError("Nested format requires 'stave_id' in clef section or exactly one stave")
...
def _parse_single_clef(clef_data: Dict[str, Any], stave_id_map: Dict[str, str]) -> Optional[Clef]:
    """
...
def _convert_nested_check_to_clef(
    ...
    for key in ["min", "max", "pattern", "query", "metric", "strategy", "lookup", "validation", "script_path", "params"]:
    ...
    if not clef_id:
        ...
        name_slug = check_data.get("name", f"{check_type}-{index}").lower().replace(" ", "-")
