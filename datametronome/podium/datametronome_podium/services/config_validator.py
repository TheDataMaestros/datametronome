...
class ConfigurationIssue:
    ...
    def __init__(
        ...
        self.issue_type = issue_type  # "conflict", "missing", "inconsistent", "deprecated"
    ...
    def __str__(self) -> str:
        ...
        items_str = f" (affects: {', '.join(self.affected_items)})" if self.affected_items else ""
...
class ConfigurationValidator:
    ...
    def _check_schedule_conflicts(self):
        ...
        for stave_id, clefs in self.clefs_by_stave.items():
            if len(clefs) > 5:  # If many checks on one stave
                ...
                if len(set(schedules)) < len(schedules) * 0.3:  # Too few unique schedules
                    stave_name = self.staves_by_id[stave_id].name
                    self.issues.append(ConfigurationIssue(
                        severity="info",
                        issue_type="conflict",
    ...
    def _check_data_type_inconsistencies(self):
        ...
        for clef in self.clefs_by_id.values():
            ...
            if stave.data_source_type == "redis" and clef.check_type in ["null_check", "range_check"]:
                self.issues.append(ConfigurationIssue(
                    severity="warning",
            ...
            if stave.data_source_type == "sqlite" and clef.check_type == "freshness_check":
                self.issues.append(ConfigurationIssue(
                    severity="info",
                    issue_type="inconsistent",
    ...
    def _check_deprecated_patterns(self):
        ...
        for stave in self.staves_by_id.values():
            ...
            for key, value in config.items():
                if "password" in key.lower() and isinstance(value, str) and not value.startswith("${"):
                    self.issues.append(ConfigurationIssue(
                        severity="warning",
            ...
            if "localhost" in str(config.get("host", "")) and ("prod" in stave.name.lower() or "production" in stave.name.lower()):
                self.issues.append(ConfigurationIssue(
                    severity="warning",
                    issue_type="inconsistent",
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if total_staves == 0:
            recommendations.append("No staves configured - consider adding data sources to monitor")
        elif total_staves == 1:
            recommendations.append("Single stave configuration - consider adding more data sources for comprehensive monitoring")
        
        if total_clefs == 0:
            recommendations.append("No clefs configured - add data quality checks to your staves")
        elif total_clefs < total_staves * 2:
            recommendations.append("Consider adding more data quality checks - aim for 2-5 checks per stave")
        
        # Check for diversity in check types
        check_types = set(c.check_type for c in self.clefs_by_id.values())
        if len(check_types) < 3:
            recommendations.append("Limited variety in check types - consider adding different types of data quality checks")
