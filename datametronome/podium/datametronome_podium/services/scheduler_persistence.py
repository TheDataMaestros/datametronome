...
class SchedulerJob:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "next_run_time": self.next_run_time.isoformat() + "Z" if self.next_run_time else None,
...
async def save_scheduler_job(job: SchedulerJob) -> bool:
    ...
    try:
        ...
        existing = await db.query({
            "sql": "SELECT id FROM scheduler_jobs WHERE id = ?",
            "params": [job.id]
        })
...
async def delete_scheduler_job(job_id: str) -> bool:
    ...
    try:
        db = await get_db()
        await db.query({
            "sql": "DELETE FROM scheduler_jobs WHERE id = ?",
            "params": [job_id]
        })
...
async def update_job_run_time(job_id: str, last_run: Optional[datetime] = None, next_run: Optional[datetime] = None) -> bool:
    """
...
async def increment_job_execution_count(job_id: str, success: bool = True) -> bool:
    ...
    try:
        ...
        await db.query({
            "sql": sql,
            "params": [datetime.utcnow().isoformat() + "Z", job_id]
        })
