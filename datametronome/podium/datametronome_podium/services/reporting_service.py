...
class ReportingService:
    ...
    async def generate_console_report(self, days_back: int = 7) -> str:
        ...
        try:
            ...
            report.append(
                f"Last Check: {health.last_check_time.strftime('%Y-%m-%d %H:%M:%S')}"
            )
            ...
            if anomalies:
                ...
                for anomaly in anomalies[:10]:  # Show last 10
                    ...
                    report.append(
                        f"{severity_icon} {anomaly.anomaly_type} - {anomaly.severity}"
                    )
                    report.append(
                        f"   Table: {anomaly.table_name}.{anomaly.column_name}"
                    )
                    report.append(f"   Description: {anomaly.description}")
                    report.append(
                        f"   Detected: {anomaly.detected_at.strftime('%Y-%m-%d %H:%M:%S')}"
                    )
    ...
    async def generate_csv_report(
        self, days_back: int = 7, filename: Optional[str] = None
    ) -> str:
    ...
    async def generate_json_report(
        ...
        try:
            ...
            report_data["system_health"]["last_check_time"] = report_data[
                "system_health"
        ...
        try:
            ...
            return {
                ...
                "anomalies_summary": {
                    ...
                    for severity, anomalies in anomalies_by_severity.items()
                },
                "anomalies_by_table": {
                    table: len(anomalies)
                    for table, anomalies in anomalies_by_table.items()
                },
