...
class TestRowCountCheck:
    ...
    async def test_row_count_pass(self, executor, sqlite_stave, mock_connector):
        ...
        result = await executor._execute_row_count_check(clef, sqlite_stave, mock_connector)
    ...
    @pytest.mark.asyncio
    async def test_row_count_between_condition(self, executor, sqlite_stave, mock_connector):
        ...
        result = await executor._execute_row_count_check(clef2, sqlite_stave, mock_connector)
        assert result.status == "warn"
        ...
        mock_connector.query.return_value = [{"row_count": 6000}]
        result = await executor._execute_row_count_check(clef3, sqlite_stave, mock_connector)
        assert result.status == "warn"
...
class TestFreshnessCheck:
    ...
    async def test_freshness_pass(self, executor, sqlite_stave, mock_connector):
        ...
        result = await executor._execute_freshness_check(clef, sqlite_stave, mock_connector)
    ...
    @pytest.mark.asyncio
    async def test_freshness_string_timestamp(self, executor, sqlite_stave, mock_connector):
        ...
        assert result.status == "pass", f"Expected pass but got {result.status}. Message: {result.message}"
        ...
        result2 = await executor._execute_freshness_check(clef, sqlite_stave, mock_connector)
...
class TestColumnValuesCheck:
    ...
    @pytest.mark.asyncio
    async def test_column_values_if_null_pass(self, executor, sqlite_stave, mock_connector):
        ...
        result = await executor._execute_column_values_check(clef, sqlite_stave, mock_connector)
    ...
    @pytest.mark.asyncio
    async def test_column_values_if_null_fail(self, executor, sqlite_stave, mock_connector):
    ...
    @pytest.mark.asyncio
    async def test_column_values_if_not_unique_pass(self, executor, sqlite_stave, mock_connector):
    ...
    @pytest.mark.asyncio
    async def test_column_values_if_not_unique_fail(self, executor, sqlite_stave, mock_connector):
    ...
    @pytest.mark.asyncio
    async def test_column_values_if_not_in_pass(self, executor, sqlite_stave, mock_connector):
        ...
        assert result.metadata["allowed_values"] == ['pending', 'completed', 'cancelled']
    
    @pytest.mark.asyncio
    async def test_column_values_if_not_in_fail(self, executor, sqlite_stave, mock_connector):
        ...
        mock_connector.query.return_value = [{
            "total_rows": 1000,
            "not_in_list_rows": 15  # 15 rows with invalid status
        }]
    ...
    def test_parse_column_values_condition_if_not_in(self, executor):
        ...
        parsed = executor._parse_column_values_condition("if_not_in: ['A', 'B', 'C'] > 0")
...
class TestLevel1ChecksErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_row_count_missing_table(self, executor, sqlite_stave, mock_connector):
    ...
    @pytest.mark.asyncio
    async def test_column_values_missing_config(self, executor, sqlite_stave, mock_connector):
    ...
    async def test_column_values_invalid_condition(self, executor, sqlite_stave):
        ...
        assert result.status == "fail"
        assert "parse" in result.message.lower() or "unrecognized" in result.message.lower()
    ...
    async def test_freshness_no_timestamp_data(self, executor, sqlite_stave):
        ...
        assert result.status == "warn"
        assert "no timestamp data" in result.message.lower() or "timestamp" in result.message.lower()
    ...
    async def test_freshness_invalid_timestamp(self, executor, sqlite_stave):
        ...
        mock_connector = AsyncMock()
        mock_connector.query.return_value = [{"latest_timestamp": "invalid-date-format"}]
...
class TestLevel1ChecksConditionParsing:
    ...
    def test_parse_if_not_in_malformed_list(self, executor):
        ...
        parsed = executor._parse_column_values_condition("if_not_in: [invalid syntax] > 0")
        assert parsed["type"] == "unknown"
        assert "error" in parsed
