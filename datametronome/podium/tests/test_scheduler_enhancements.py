...
class TestSchedulerPersistence:
    ...
    async def test_save_scheduler_job(self, sample_job):
        ...
        with patch('datametronome_podium.services.scheduler_persistence.get_db') as mock_get_db:
            mock_db = AsyncMock()
            ...
            mock_db.query.return_value = [
                {
                    ...
                    "started_at": (datetime.utcnow() - timedelta(hours=i)).isoformat() + "Z",
                    "completed_at": (datetime.utcnow() - timedelta(hours=i)).isoformat() + "Z"
                }
                for i in range(10)
            ]
            
            metrics = await calculate_job_health_metrics("job_clef-001", lookback_days=30)
...
class TestRetryLogic:
    ...
    async def test_execute_with_retry_on_failure(self):
        ...
        with patch('datametronome_podium.services.clef_scheduler.get_db') as mock_get_db, \
             patch('datametronome_podium.services.clef_scheduler.ClefExecutor') as mock_executor, \
             patch('datametronome_podium.services.clef_scheduler.record_job_execution') as mock_record, \
             patch('datametronome_podium.services.clef_scheduler.increment_job_execution_count') as mock_increment:
            ...
            mock_db.query.side_effect = [
                [{
                    ...
                    "fail": "< 1000",
                    "schedule": "@hourly",
                    "retry_config": json.dumps({"max_retries": 2, "backoff_factor": 1.0}),
                    "is_active": True
                }],
                [{
                    "id": "stave-001",
            ...
            # Mock store result
            with patch('datametronome_podium.services.clef_scheduler._store_clef_result'):
...
class TestSchedulerIntegration:
    ...
    async def test_full_job_lifecycle(self):
        ...
        with patch('datametronome_podium.services.scheduler_persistence.get_db') as mock_get_db:
            ...
            # Update run times
            now = datetime.utcnow()
            result = await update_job_run_time(job.id, last_run=now, next_run=now + timedelta(hours=1))
            assert result is True
