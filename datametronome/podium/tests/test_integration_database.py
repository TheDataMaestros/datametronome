...
class TestDatabasePartitioningIntegration:
    ...
    async def _setup_partitioned_tables(self, conn):
        ...
        await conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_partitioned_events_created_at ON partitioned_events(created_at)"
        )
        await conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_partitioned_events_user_id ON partitioned_events(user_id)"
        )
        await conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_partitioned_users_username ON partitioned_users(username)"
        )
        await conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_partitioned_metrics_recorded_at ON partitioned_metrics(recorded_at)"
        )
    ...
    async def test_daily_partition_insertion(self, partitioned_database):
        ...
        for i in range(15):
            ...
            partition_exists = await partitioned_database.fetchval(
                f"""
                SELECT EXISTS (
            ...
            row_count = await partitioned_database.fetchval(
                f"""
    ...
    async def test_hash_partition_distribution(self, partitioned_database):
        ...
        for i in range(4):
            partition_name = f"partitioned_users_hash_{i}"
            count = await partitioned_database.fetchval(
    ...
    async def test_partition_performance_queries(self, partitioned_database):
        ...
        logger.info(
            f"Cross-partition aggregation completed in {query2_time:.4f} seconds"
        )
...
class TestDatabaseOperationsIntegration:
    ...
    async def _setup_operational_tables(self, conn):
        ...
        # Create indexes
        await conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_operational_users_email ON operational_users(email)"
        )
        await conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_operational_orders_user_id ON operational_orders(user_id)"
        )
        await conn.execute(
        ...
        try:
            async with operational_database.transaction():
                ...
                """,
                    ...
                    "ORD-002",
                    -50.00,
                )  # Negative amount violates constraint
