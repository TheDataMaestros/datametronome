...
class TestTDDCompliantClefs:
    ...
    def test_tdd_level_3_advanced_declarative_checks(self):
        ...
        print(f"   ✅ {lookup_clef.name}: {lookup_clef.check_type} (Level {lookup_clef.level})")
    ...
    def test_tdd_level_4_custom_code(self):
        ...
        print(f"   ✅ {python_clef.name}: {python_clef.check_type} (Level {python_clef.level})")
    ...
    def test_condition_string_examples(self):
        ...
        condition_examples = [
            ...
            ("if_out_of_range: [0, 150] > 1%", "If out of range percentage greater than 1%"),
            ("if_not_matching: '^[a-z0-9._%+-]+@[a-z0-9.-]+\\.[a-z]{2,}$' > 5%", "If not matching pattern percentage greater than 5%")
        ]
    ...
    def test_complete_tdd_workflow_example(self):
        ...
        for level, count in level_counts.items():
            if count > 0:
                level_desc = ["", "Declarative", "Intelligent", "Advanced Declarative", "Custom Code"][level]
        ...
        print(f"      • Clear severity mapping (pass/warn/fail → Harmony/Dissonance/Cacophony)")
