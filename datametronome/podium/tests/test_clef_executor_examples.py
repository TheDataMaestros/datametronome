...
class TestClefExecutorExamples:
    ...
    def test_execute_null_check_example(self, mock_db_connector):
        ...
        mock_db_connector.query.return_value = [
            {"total_rows": 1000, "non_null_rows": 990, "null_rows": 10}
        ]
    ...
    def test_execute_row_count_check_example(self, mock_db_connector):
        ...
        print(
            f"   Conditions: warn={result.details['warn_condition']} fail={result.details['fail_condition']}"
        )
    ...
    def test_execute_row_count_check_failure_example(self, mock_db_connector):
        ...
        mock_db_connector.query.return_value = [{"row_count": 50}]  # Too few orders!
    ...
    def test_execute_column_values_warn_example(self, mock_db_connector):
        ...
        mock_db_connector.query.return_value = [
            {"total_rows": 100, "non_null_rows": 90, "null_rows": 10}
        ]
    ...
    def test_execute_column_values_fail_example(self, mock_db_connector):
        ...
        mock_db_connector.query.return_value = [
            {"total_rows": 50, "non_null_rows": 30, "null_rows": 20}
        ]
    ...
    def test_execute_multiple_clefs_example(self, mock_db_connector):
        ...
        def mock_query(sql, *args, **kwargs):
            ...
            if (
                "from users" in normalized_sql
                and "count(*) - count(email)" in normalized_sql
            ):
    ...
    def test_clef_executor_stats_example(self, mock_db_connector):
        ...
        # Create test data
        stave = create_postgres_stave(
            name="Test Database", host="localhost", database="test", user="test"
        )

        clefs = [
            create_null_check(
                stave_id=stave.id, name="Check 1", table="table1", column="col1"
            ),
            create_null_check(
                stave_id=stave.id, name="Check 2", table="table2", column="col2"
            ),
            create_null_check(
                stave_id=stave.id, name="Check 3", table="table3", column="col3"
            ),
        ]

        # Mock responses
        mock_db_connector.query.return_value = [
            {"total_rows": 100, "non_null_rows": 100, "null_rows": 0}
        ]
