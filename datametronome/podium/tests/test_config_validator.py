...
class TestConfigurationValidatorExamples:
    ...
    def test_detect_duplicate_ids(self):
        ...
        stave1 = create_postgres_stave(
            name="DB 1", host="localhost", database="db1", user="user"
        )
        stave1.id = "duplicate-id"

        stave2 = create_postgres_stave(
            name="DB 2", host="localhost", database="db2", user="user"
        )
    ...
    def test_detect_connection_conflicts(self):
        ...
        for issue in result["issues"]:
            if (
                ...
                and "same PostgreSQL database" in issue.message
            ):
    ...
    def test_detect_range_check_conflicts(self):
        ...
        stave = create_postgres_stave(
            name="Test DB", host="localhost", database="test", user="user"
        )
    ...
    def test_detect_inappropriate_check_types(self):
        ...
        # Create Redis stave
        stave = create_redis_stave(name="Cache", host="redis.example.com")
    ...
    def test_detect_schedule_conflicts(self):
        ...
        # Create a stave
        stave = create_postgres_stave(
            name="Busy DB", host="localhost", database="busy", user="user"
        )
    ...
    def test_detect_hardcoded_passwords(self):
        ...
        for issue in result["issues"]:
            if (
                ...
                and "hardcoded password" in issue.message
            ):
    ...
    def test_comprehensive_validation_example(self):
        ...
        staves = [
            # Good stave
            create_postgres_stave(
                name="Good DB", host="good.example.com", database="good", user="monitor"
            ),
            # Stave with hardcoded password
            create_postgres_stave(
                name="Bad DB", host="bad.example.com", database="bad", user="monitor"
            ),
        ]
        ...
        clefs = [
            # Good clef
            create_null_check(
                stave_id=staves[0].id, name="Good Check", table="users", column="email"
            ),
            # Clef referencing non-existent stave
            create_null_check(
                stave_id="nonexistent", name="Bad Check", table="users", column="email"
            ),
        ]
...
class TestYAMLValidationExamples:
    ...
    def test_validate_conflicting_yaml(self, tmp_path):
        ...
        example_file = (
            Path(__file__).parent.parent / "examples" / "conflicting-config.yaml"
        )
...
class TestValidatorOutputExamples:
    ...
    def test_validation_summary_example(self):
        ...
        issues = [
            ConfigurationIssue(
                "error", "conflict", "Duplicate ID found", ["DB 1", "DB 2"]
            ),
            ConfigurationIssue(
                "warning", "conflict", "Same connection used", ["DB 3", "DB 4"]
            ),
            ConfigurationIssue(
                "info",
                "performance",
                "Many checks scheduled",
