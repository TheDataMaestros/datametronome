...
class TestLevel1ChecksE2E:
    ...
    async def test_row_count_check_e2e(
        self, mock_insert, mock_get_db, async_client, 
        mock_db, test_stave_data, test_clef_row_count
    ):
        ...
        with patch('datametronome_podium.services.clef_executor.ConnectionTester') as mock_conn_tester:
    ...
    async def test_column_values_if_null_e2e(
        self, mock_insert, mock_get_db, async_client,
        mock_db, test_stave_data, test_clef_column_values
    ):
    ...
    async def test_column_values_if_not_in_e2e(
        ...
        assert data["metadata"]["allowed_values"] == ['pending', 'completed', 'cancelled']
    ...
    async def test_freshness_check_e2e(
        self, mock_insert, mock_get_db, async_client,
        mock_db, test_stave_data, test_clef_freshness
    ):
