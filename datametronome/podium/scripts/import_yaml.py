...
async def clean_existing_items(stave_ids: List[str], clef_ids: List[str]):
    ...
    if clef_ids:
        ...
        for clef_id in clef_ids:
            ...
            await db.execute(
                "DELETE FROM checks WHERE clef_id = ?",
                [clef_id]
            )
            # Delete the clef
            await db.execute(
    ...
    if stave_ids:
        ...
        for stave_id in stave_ids:
            ...
            await db.execute(
                "DELETE FROM checks WHERE stave_id = ?",
                [stave_id]
            )
...
async def import_yaml_config(yaml_file: str, clean: bool = False, dry_run: bool = False):
    ...
    if dry_run:
        ...
        for c in clefs_config:
            logger.info(f"  - {c.get('name')} (ID: {c.get('id', 'auto-generated')}, Type: {c.get('check_type')})")
        logger.info("")
...
async def main():
    ...
    if clean:
        logger.info("🔄 CLEAN mode enabled - will delete existing items with matching IDs")
        logger.info("")
