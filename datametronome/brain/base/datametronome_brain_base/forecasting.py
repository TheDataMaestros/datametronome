...
class ForecastResult(BaseModel):
    ...
    confidence_level: float = Field(ge=0.0, le=100.0, description="Confidence level (0-100)")
...
class SarimaForecaster:
    ...
    def train(self, data: pd.Series | list[float] | np.ndarray) -> dict[str, Any]:
        ...
        # Validate data
        if len(data) < max(self.order[0] + self.order[2], self.seasonal_order[0] + self.seasonal_order[2] + self.seasonal_order[3]):
            raise ValueError(
        ...
        try:
            ...
            logger.info(f"SARIMA model trained successfully. AIC: {model_info['aic']:.2f}")
    ...
    def detect_anomaly(
        ...
        """
        # Generate forecast
        forecast_result = self.forecast(steps=1, confidence_level=confidence_level, alpha=alpha)
        ...
        if is_anomaly:
            ...
            p_value = max(0.0, min(1.0, (1.0 - confidence_level / 100.0) * (1.0 - normalized_distance)))
        else:
    ...
    def auto_select_order(
        ...
        logger.info(f"Best model: order={best_order}, seasonal_order={best_seasonal_order}, {information_criterion}={best_score:.2f}")
