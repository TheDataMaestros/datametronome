...
class DataProfiler:
    ...
    def _add_categorical_stats(self, profile: ColumnProfile, data: pd.Series) -> None:
        ...
        profile.top_value_count = (
            int(value_counts.iloc[0]) if len(value_counts) > 0 else None
        )
        profile.top_value_percentage = (
            (value_counts.iloc[0] / len(clean_data)) * 100
    ...
    def _add_datetime_stats(self, profile: ColumnProfile, data: pd.Series) -> None:
        ...
        profile.most_common_year = (
            int(clean_data.dt.year.mode().iloc[0])
            if len(clean_data.dt.year.mode()) > 0
            else None
        )
        profile.most_common_month = (
            int(clean_data.dt.month.mode().iloc[0])
            if len(clean_data.dt.month.mode()) > 0
            else None
        )
        profile.most_common_day_of_week = (
            int(clean_data.dt.dayofweek.mode().iloc[0])
            if len(clean_data.dt.dayofweek.mode()) > 0
            else None
        )
    ...
    def _generate_table_summary(
        self, data: pd.DataFrame, column_profiles: dict[str, ColumnProfile]
    ) -> dict[str, Any]:
