...
class DriftResult(BaseModel):
    ...
    critical_p_value: float = Field(
        ge=0.0, le=1.0, description="Critical p-value threshold"
    )
    ...
    metadata: dict[str, Any] = Field(
        default_factory=dict, description="Additional test information"
    )
...
class DriftDetector:
    ...
    def kolmogorov_smirnov_test(
        ...
        if not 0 < critical_p_value <= 1:
            raise ValueError(
                f"Critical p-value must be between 0 and 1, got {critical_p_value}"
            )
