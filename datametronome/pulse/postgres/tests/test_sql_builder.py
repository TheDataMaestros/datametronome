...
class TestPostgresSQLBuilder:
    ...
    def test_delete_using_values_asyncpg_multi_col_single_row(self):
        ...
        sql = self.builder.delete_using_values_asyncpg(
            "public.events", ["id", "source"], 1
        )
        assert "DELETE FROM public.events" in sql
    ...
    def test_delete_using_values_asyncpg_complex_columns(self):
        ...
        assert (
            "USING (VALUES ($1, $2, $3)) AS v(user_id, event_type, created_at)" in sql
        )
        assert (
    ...
    def test_sql_injection_prevention(self):
        ...
        sql = self.builder.delete_using_values_asyncpg(
            "public.events", dangerous_columns, 1
        )
...
class TestPostgresSQLBuilderIntegration:
    ...
    def test_full_delete_workflow(self):
        ...
        delete_sql = builder.delete_using_values_asyncpg(
            "public.user_sessions", ["session_id", "user_id"], 50
        )
