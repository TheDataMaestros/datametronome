...
class PostgresWriteOnlyPulse(Pulse, Writable):
    ...
    async def replace_using_temp(
        ...
        async with self._pool.acquire() as conn:
            async with conn.transaction():
                # 1) Create temp table
                await conn.execute(
                    self._sql.create_temp_table_like(temp_table, destination)
                )

                # 2) Bulk copy into temp
                records = [tuple(record[col] for col in columns) for record in data]
                await conn.copy_records_to_table(
                    temp_table, records=records, columns=columns
                )

                # 3) Delete matching rows
                await conn.execute(
                    self._sql.delete_using_temp(destination, temp_table, key_columns)
                )

                # 4) Insert from temp
                await conn.execute(
                    self._sql.insert_from_temp(destination, temp_table, columns)
                )
    ...
    async def ensure_range_partition(
        ...
        async with self._pool.acquire() as conn:
            await conn.execute(
                self._sql.create_range_partition(
                    parent_table, partition_table, from_value, to_value
                )
            )
    ...
    async def ensure_list_partition(
        ...
        async with self._pool.acquire() as conn:
            await conn.execute(
                self._sql.create_list_partition(parent_table, partition_table, values)
            )
