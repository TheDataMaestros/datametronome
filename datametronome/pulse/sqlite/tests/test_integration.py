...
class TestSQLitePulseIntegration:
    ...
    async def test_transaction_handling(self, sqlite_pulse):
        ...
        await sqlite_pulse.write(
            [{"name": "Alice", "email": "alice@example.com", "active": True}], "users"
        )
        ...
        try:
            ...
            await sqlite_pulse.write(
                [{"name": "Bob", "email": "alice@example.com", "active": True}], "users"
            )
...
class TestSQLitePerformance:
    ...
    async def test_bulk_insert_performance(self, temp_db_path):
        ...
        large_dataset = [{"name": f"Item{i}", "value": i} for i in range(10000)]
        ...
        # Verify all records were inserted
        count_result = await pulse.query(
        ...
        assert (
            duration < 10.0
        ), f"Bulk insert took {duration:.2f} seconds, expected < 10s"
    ...
    async def test_query_performance(self, temp_db_path):
        ...
        # Insert test data
        test_data = [
            {"category": f"cat{i % 10}", "value": float(i)} for i in range(10000)
        ]
        ...
        assert (
            duration < 5.0
        ), f"Complex query took {duration:.2f} seconds, expected < 5s"
