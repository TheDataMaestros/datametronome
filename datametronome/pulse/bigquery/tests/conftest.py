...
def get_bigquery_config():
    ...
    if credentials_json_str:
        try:
            ...
            pytest.skip("Invalid BIGQUERY_TEST_CREDENTIALS_JSON format")  # ty: ignore

    if not project_id:
        pytest.skip("BIGQUERY_TEST_PROJECT_ID environment variable not set")  # ty: ignore
...
def test_dataset_id(bigquery_config, test_client):
    ...
    pytest.skip("No accessible datasets found (neither in project nor public datasets)")  # ty: ignore
...
def test_dataset(bigquery_config, test_client, test_dataset_id):
    ...
    if "." in test_dataset_id and "bigquery-public-data" in test_dataset_id:
        ...
        try:
            ...
            pytest.skip(f"Could not access dataset {test_dataset_id}: {e}")  # ty: ignore
