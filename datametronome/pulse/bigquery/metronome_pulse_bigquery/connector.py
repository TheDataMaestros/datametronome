...
class BigQueryPulse(Pulse, Readable, Writable):
    ...
    async def query(self, query_config) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            query_job = await loop.run_in_executor(
                None,
                lambda: self._client.query(sql, job_config=self._get_job_config(params))
            )
    ...
    async def write(self, data: List[Dict], destination: str, config: Optional[Dict] = None) -> None:
        """Write data to BigQuery table.
        
        Args:
            data: List of dictionaries to write
            destination: Target table name (can include dataset: 'dataset.table')
            config: Optional configuration for write operation
        """
        if not await self.is_connected():
            raise RuntimeError("Not connected to BigQuery")
        ...
        try:
            # Parse destination (dataset.table or just table)
            if '.' in destination:
            ...
            if not dataset_id:
                raise ValueError("Dataset must be specified in destination or constructor")
    ...
    async def get_table_info(self, table_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            table = await loop.run_in_executor(
                None,
                lambda: self._client.get_table(f"{self._project_id}.{dataset_id}.{table_id}")
            )
    ...
    async def list_tables(self, dataset: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            tables = await loop.run_in_executor(
                None,
                lambda: list(self._client.list_tables(dataset_id))
            )
    ...
    def _get_job_config(self, params: Optional[List] = None) -> Optional[bigquery.QueryJobConfig]:
        ...
        if params:
            ...
            # For now, we'll use legacy SQL with parameters
