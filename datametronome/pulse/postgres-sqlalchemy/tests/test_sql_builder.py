import pytest
from metronome_pulse_postgres_sqlalchemy.sql_builder import PostgresSQLAlchemyBuilder
...
class TestPostgresSQLAlchemyBuilder:
    ...
    def test_delete_using_values_sqlalchemy_build_complex_columns(self):
        ...
        sql = self.builder.delete_using_values(
            "public.user_events", ["user_id", "event_type", "created_at"], 1
        )
        assert "DELETE FROM public.user_events" in sql
        assert (
            "USING (VALUES (:user_id_0, :event_type_0, :created_at_0)) AS v(user_id, event_type, created_at)"
            in sql
        )
        assert (
            "t.user_id = v.user_id AND t.event_type = v.event_type AND t.created_at = v.created_at"
            in sql
        )
    ...
    def test_column_name_handling(self):
        ...
        sql = self.builder.delete_using_values(
            "public.events", ['"user_id"', '"event_type"'], 1
        )
        ...
        sql = self.builder.delete_using_values(
            "public.events", ["user-id", "event-type"], 1
        )
    ...
    def test_sql_injection_prevention(self):
        ...
        assert (
            "USING (VALUES (:id_0, :'; DROP TABLE users; --_0)) AS v(id, '; DROP TABLE users; --)"
            in sql
        )
    ...
    def test_parameter_naming_convention(self):
        ...
        sql = self.builder.delete_using_values(
            "public.events", ["user_id", "event_type"], 2
        )
...
class TestPostgresSQLAlchemySQLBuilderIntegration:
    ...
    def test_full_delete_workflow(self):
        ...
        delete_sql = builder.delete_using_values(
            "public.user_sessions", ["session_id", "user_id", "created_at"], 50
        )
        ...
        assert (
            "t.session_id = v.session_id AND t.user_id = v.user_id AND t.created_at = v.created_at"
            in delete_sql
        )
    ...
    def test_parameter_binding_compatibility(self):
        ...
        sql = builder.delete_using_values(
            "public.analytics", ["user_id", "event_date", "metric_value"], 10
        )
